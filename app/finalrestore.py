...
y_data = [
    [261.1, 289.04, 458.87],
    [307.88, 329.83, 458.87],
    [352.62, 367.82, 761.88],
    [395.68, 403.59, 924.41],
    [329.14, 379.26, 788.18],
    [294.40, 359.46, 620.64],
    [426.05, 468.97, 1207.99],
    [389.27, 452.65, 1025.01],
    [389.27, 454.27, 1148.53],
    [439.94, 503.71, 1400.99],
    [318.09, 412.55, 744.40],
    [372.53, 465.86, 978.46],
    [445.31, 521.67, 1544.02],
    [415.71, 508.82, 1317.12],
    [379.79, 487.55, 1084.11],
    [337.27, 457.27, 849.14],
]
a1 = 80
b1 = 4
c1 = 3
FX = np.exp(
    np.log(248.945)
    + 0.135 * np.log(a1)
    + 0.147 * (np.log(b1) + np.log(0.001))
    + 0.62 * np.log(c1)
)
FY = np.exp(
    np.log(317.26)
    + 0.258 * np.log(a1)
    + 0.215 * (np.log(b1) + np.log(0.001))
    + 0.416 * np.log(c1)
)
FZ = np.exp(
    np.log(110.728)
    + 0.407 * np.log(a1)
    + 0.109 * (np.log(b1) + np.log(0.001))
    + 1.16 * np.log(c1)
)
y_dataOR = [
    [FX, FY, FZ],
...
with tf.Session() as sess:
    ...
    # abschlussproject\\3input\\save3inputnetBEST.ckpt",
    # )

...
INIT_PARAMS = [[0.3, 0.3], [0.2, 0.2], [10, -10]][2]
...
ax = Axes3D(fig)
a3D, b3D = np.meshgrid(
    np.linspace(-10, 10, 30), np.linspace(-10, 2, 30)
)  # parameter space
cost3D = np.array(
    [
...
ax.scatter(
    a_list[0], b_list[0], zs=cost_list[0], s=300, c="r"
)  # initial parameter place
ax.set_xlabel("a")
ax.set_ylabel("b")
ax.plot(
    a_list, b_list, zs=cost_list, zdir="z", c="r", lw=3

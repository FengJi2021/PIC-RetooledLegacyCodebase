...
y_data = np.exp(np.log(248.945) + 0.135*np.log(x1_data) + 0.147*(np.log(x2_data)+np.log(0.001)) + 0.62*np.log(2)) 
...
# saver = tf.train.import_meta_graph("C:\\Users\\admin\\Desktop\\abschlussproject\\NeuralSaver\\savenet.ckpt.meta")

with tf.Session() as sess:
    saver.restore(sess,"C:\\Users\\admin\\Desktop\\abschlussproject\\2input\\savenet2.ckpt")
...
#     print(prediction_value)
    ...
    fig = plt.figure()
    ax = Axes3D(fig)
    ax.plot_surface(x1_data1D.reshape(samplesize,samplesize),x2_data1D.reshape(samplesize,samplesize),prediction_value.reshape(samplesize,samplesize))
    plt.show()

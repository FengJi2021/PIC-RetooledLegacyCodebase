...
class start_main(QtGui.QMainWindow):
    def __init__(self, parent=None):
        ...
        self.ui.updateMTstaticinfo.triggered.connect(
            self.showupdateMTstaticinfo
        )
        ...
        self.ui.NeuralnetworkVisual.triggered.connect(
            self.showNeuralnetworkVisual
        )
...
class start_login(QtGui.QWidget):
    def __init__(self, parent=None):
        ...
        QtCore.QObject.connect(
            self.ui.btLogin, QtCore.SIGNAL("clicked()"), self.login
        )
        QtCore.QObject.connect(
            self.ui.btExit, QtCore.SIGNAL("clicked()"), self.close
        )
...
class start_userinfo(QtGui.QWidget):
    def __init__(self, parent=None):
        ...
        QtCore.QObject.connect(
            self.ui.CHAXUN, QtCore.SIGNAL("clicked()"), self.getData
        )
        QtCore.QObject.connect(
            self.ui.GENGXIN, QtCore.SIGNAL("clicked()"), self.insertData
        )
        QtCore.QObject.connect(
            self.ui.SHANCHU, QtCore.SIGNAL("clicked()"), self.deleteData
        )
        # initial my table
    ...
    def getData(self):
        ...
        if APPROVE == str("最高权限"):  # hightest autority
            ...
            sql_str = (
                "select * from userinfo where userid='" + str(APPROVE) + "'"
            )
            print(sql_str)
...
class start_getMTstaticinfo(QtGui.QWidget):
    ...
    def getData(self):
        ...
        f = open(
            "C:\\Users\\admin\\Desktop\\abschlussproject\\getforcetest.txt",
            mode="r",
        )
        for each_line in f:
            ...
            FZ = FZ[:-1]
            mysql_client.executeSql(
                "INSERT INTO realtimestate VALUES ('"
                + mytime
                + "','1','"
                + FX
                + "','"
...
class start_updateMTstaticinfo(QtGui.QWidget):
    def __init__(self, parent=None):
        ...
        self.ui.xuhao.setFocus()
        QtCore.QObject.connect(
            self.ui.gengxin, QtCore.SIGNAL("clicked()"), self.updateDate
        )
    ...
    def updateDate(self):
        ...
        sql_str7 = (
            "UPDATE machinetool SET XFeedspeed ='"
            + str(list[6])
            + "' WHERE id= '"
            + str(id)
            + "'"
        ...
        sql_str8 = (
            "UPDATE machinetool SET YFeedspeed ='"
            + str(list[7])
            + "' WHERE id= '"
            + str(id)
            + "'"
        ...
        sql_str9 = (
            "UPDATE machinetool SET ZFeedspeed ='"
            + str(list[8])
            + "' WHERE id= '"
            + str(id)
            + "'"
        ...
        sql_str10 = (
            "UPDATE machinetool SET Controlsys ='"
            + str(list[9])
            + "' WHERE id= '"
            + str(id)
            + "'"
        ...
        sql_str11 = (
            "UPDATE machinetool SET Workinghour ='"
            + str(list[10])
            + "' WHERE id= '"
            + str(id)
            + "'"
        ...
        sql_str12 = (
            "UPDATE machinetool SET Alarmnumber ='"
            + str(list[11])
            + "' WHERE id= '"
            + str(id)
            + "'"
...
class start_craftprocessdata(QtGui.QWidget):
    def __init__(self, parent=None):
        ...
        QtCore.QObject.connect(
            ...
            self.insertcraftData,
        )
        QtCore.QObject.connect(
            self.ui.LURU, QtCore.SIGNAL("clicked()"), self.insertprocessData
        )
    ...
    def getData(self):
        ...
        self.ui.GONGXU.setHorizontalHeaderLabels(
            ["工序号", "工序内容", "工艺卡编号", "加工设备"]
        )  # production serie nummer,content,machine
        self.ui.GONGXU.setColumnWidth(1, 450)
        self.ui.GONGXU.setColumnWidth(2, 90)
        self.ui.GONGXU.setColumnWidth(3, 80)
        for i in range(0, Rownumber):
            row = i
            self.ui.GONGXU.setItem(
                row, 0, QTableWidgetItem(str(data4process[i][2]))
            )
            self.ui.GONGXU.setItem(
                row, 1, QTableWidgetItem(str(data4process[i][1]))
            )
            self.ui.GONGXU.setItem(
                row, 2, QTableWidgetItem(str(data4process[i][0]))
            )
            self.ui.GONGXU.setItem(
                row, 3, QTableWidgetItem(str(data4process[i][3]))
            )
    ...
    def insertprocessData(self):
        ...
        sql_str = (
            "INSERT INTO processdata VALUES ('"
            + craftid
            + "','"
            + content
            + "','"
...
class start_realtimestate(QtGui.QWidget):
    ...
    def getData(self):
        ...
        self.ui.SHOULISHUJU.setHorizontalHeaderLabels(
            ["时间(S)", "Fx受力(N)", "Fy受力(N)", "Fz受力(N)"]
        )  # time,force-x,y,z
        ...
        for i in range(0, Rownumber):
            row = i
            self.ui.SHOULISHUJU.setItem(
                row, 0, QTableWidgetItem(str(data[i][0]))
            )
            self.ui.SHOULISHUJU.setItem(
                row, 1, QTableWidgetItem(str(data[i][2]))
            )
            self.ui.SHOULISHUJU.setItem(
                row, 2, QTableWidgetItem(str(data[i][3]))
            )
            self.ui.SHOULISHUJU.setItem(
                row, 3, QTableWidgetItem(str(data[i][4]))
            )
...
class start_productpicture(QtGui.QWidget):
    def __init__(self, parent=None):
        ...
        self.ui = Ui_Design()
        self.ui.setupUi(self)
        self.ui.GONGYIHAO.setPlaceholderText(
            "请输入工艺编号"
        )  # production serie nummer
    ...
    def getData(self):
        ...
        sql_str = (
            "select * from craftandcaddata where craftnumber="
            + str(craftnumber)
            + " and processnumber= "
            + str(processnumber)
        )
...
class start_NeuralnetworkVisual(QtGui.QWidget):
    def __init__(self, parent=None):
        ...
        self.ui.setupUi(self)
        QtCore.QObject.connect(
            self.ui.XIANSHI, QtCore.SIGNAL("clicked()"), self.showVisual
        )
        QtCore.QObject.connect(
            self.ui.XIANSHI_2, QtCore.SIGNAL("clicked()"), self.showResult
        )
        samplesize = 20
        ...
        self.ui.SHUJUBIAO.setHorizontalHeaderLabels(
            ["Fx受力(N)", "主轴转速(m/min)", "进给速度(mm/s)", "吃刀量(mm)"]
        )  # force,main-speed,feedspeed,cuttingdepth
        ...
        for i in range(0, rownumber):
            ...
            self.ui.SHUJUBIAO.setItem(
                row, 1, QTableWidgetItem(str(x1_data[i]))
            )
            self.ui.SHUJUBIAO.setItem(
                row, 2, QTableWidgetItem(str(x2_data[i]))
            )
            self.ui.SHUJUBIAO.setItem(
                row, 3, QTableWidgetItem(str(x3_data[i]))
            )
    ...
    def showVisual(self):
        ...
        def NeuralWorking(INITME):
            ...
            a, b = [
                tf.Variable(initial_value=p, dtype=tf.float32)
        ...
        cost3D = np.array(
            [
                np.mean(np.square(y_fun(a_, b_) - y))
    ...
    def showResult(self):
        ...
        self.ui.JIEGUO.setHorizontalHeaderLabels(
            ["误差均方值", "A的取值", "B的取值", "C的取值", "D的取值"]
        )  # varianze
        ...
        # tensorflow graph
        a, b, c, d = [
            tf.Variable(initial_value=p, dtype=tf.float32) for p in INIT_PARAMS
        ]
